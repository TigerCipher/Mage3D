...
mage::DebugInfo::DebugInfo()
{
    ...
    const auto debugInterface = reinterpret_cast<DXGIGetDebugInterface>(reinterpret_cast<void*>(GetProcAddress(
            dxgidll, "DXGIGetDebugInterface")));
    if (!debugInterface)
    {
        throw DISPLAY_LAST_EXCEPTION();
    }
...
list<std::string> mage::DebugInfo::getMessages() const
{
    ...
    for (auto i = m_next; i < end; i++)
    {
        HRESULT hr;
        SIZE_T msgLen;
        GFX_THROW_FAILED(m_infoQueue->GetMessageA(DXGI_DEBUG_ALL, i, nullptr, &msgLen));
        auto bytes = createScope<byte[]>(msgLen);
        auto msg = reinterpret_cast<DXGI_INFO_QUEUE_MESSAGE*>(bytes.get());
        GFX_THROW_FAILED(m_infoQueue->GetMessageA(DXGI_DEBUG_ALL, i, msg, &msgLen));
